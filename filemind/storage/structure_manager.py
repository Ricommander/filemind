...
def get_target_subfolder(base_folder: Path, year: int) -> Path:
    ...
    # Validierung
    if base_folder.name not in BASE_FOLDERS:
        logger.warning(
    ...
    try:
        ...
        error_msg = f"Fehler beim Finden des Zielordners für {year} in {base_folder}: {e}"
        logger.error(error_msg)
        raise RuntimeError(error_msg) from e
...
def _get_next_subfolder_number(base_folder: Path, year: int) -> int:
    ...
    next_number = existing_subfolders[-1][0] + 1 if existing_subfolders else 1
