...
def _extract_video_metadata_name(path: Path) -> str:
	...
	logger.debug(
		f"Video-Metadaten (Stub): {path.name} -> {name} "
...
def _extract_audio_metadata_name(path: Path) -> str:
	...
	logger.debug(
		f"Audio-Metadaten (Stub): {path.name} -> {name} "
		f"(Größe: {file_size_kb:.1f} KB)"
	)
...
def _extract_archive_metadata_name(path: Path) -> str:
	...
	logger.debug(
		f"Archiv-Metadaten (Stub): {path.name} -> {name} "
		f"(Größe: {file_size_mb:.1f} MB)"
	)
...
def _extract_generic_metadata_name(path: Path) -> str:
	...
	logger.debug(
		f"Generische Metadaten: {path.name} -> {name} "
		f"(Größe: {size_str})"
	)
